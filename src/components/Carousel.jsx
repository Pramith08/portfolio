...
export const Carousel = ({data}) => {
  ...
  return (
    <div className='mb-6 relative flex justify-center items-center w-4/6 h-auto'>
      <BsArrowLeftCircleFill className='absolute -left-10 lg:left-0 w-7 h-7 fill-neutral-300 bg-transparent hover:cursor-pointer' onClick={previousSlide}/>
      {data.map((item,idx)=> {
        return <img src={item} alt="" key={idx} className={slide === idx ? "w-auto h-full" : "hidden"}/>
       })
      }
      <BsArrowRightCircleFill className='absolute -right-10 lg:right-0 w-7 h-7 fill-neutral-300 bg-transparent hover:cursor-pointer'  onClick={nextSlide}/>
